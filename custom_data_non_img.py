############################## PREPARATION OF THE CSV AND TOY DATASETS ##############################
...
def get_circle_data(sample, noise):
    feature, labels = make_circles(n_samples=sample, noise=noise)
...
def get_moons_data(samples, noise):
    feature, labels = make_moons(n_samples=samples, noise=noise)
    X_train, Y_train, X_val, Y_val, X_test, Y_test = train_dev_test_split(feature, labels, 0.3)
