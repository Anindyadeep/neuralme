...
def predict_plot(params, X_val, Y_val):
    fig = plot_decision_boundary(lambda x: predict(params, x.T), X_val, Y_val)
    return fig


def predict(params, X_test):
    A_test, cache_test = full_feed_forward(X_test, params)
    predictions = np.round(A_test)
    return predictions


# test part 3

def train(back_prop, stream=False):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 1000 == 0 and acc_val < 0.9:
            learning_rate * 10
        ...
        if stream and epoch % 100 == 0:
            st.write("#### At epoch: ", epoch)
            st.write("TRAIN LOSS: ", loss_train, "TRAIN ACC: ", acc_train)
            st.write("VAL LOSS: ", loss_val, "VAL ACC: ", acc_val)

    return params, LOSS_TRAIN, ACC_TRAIN, LOSS_VAL, ACC_VAL, EPOCHS
